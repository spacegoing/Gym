# file: src/validators/validator_en.py
...
import requests
from data_loader import DEFINITION_GENERATOR_SYSTEM_PROMPT, JUDGE_SYSTEM_PROMPT, PROMPT_VALIDATION_JUDGE_SYSTEM_PROMPT, THINKING_VALIDATION_JUDGE_SYSTEM_PROMPT, THINKING_VALIDATION_JUDGE_SYSTEM_PROMPT, template_json, conflict_dict, LLM_INSTRUCTIONS, eval_modes, subinst_def, inst_def, LLM_JUDGE_QUESTION_PROMPT
...
EXPECTED_ARGUMENTS = {
    ...
    # --- REMAINDER (original order, excluding priority keys) ---
...
def validate_instruction(response: str, inst_type: str, kwargs: Dict[str, Any], all_instructions: Dict = None) -> Tuple[bool, str]:
    ...
    try:
        ...
        if inst_type == "detectable_format:number_paragraphs":
            ...
            if relation == "equal to":
                expected_desc = f"equal to {num_paragraphs}"
            elif relation == "at least":
                expected_desc = f"at least {num_paragraphs}"
            else:  # relation == "less than"
                expected_desc = f"less than {num_paragraphs}"
            
            message = f"Found {actual_paragraph_count} paragraphs. Expected {expected_desc}."
...
def validate_llm_instruction(
    ...
    return base_validator.validate_llm_instruction(
        response, inst_type, kwargs, all_instructions, definition_cache, language="en"
    )
...
def validate_prompt_against_instructions(user_prompt: str, turn_instructions: Dict) -> Tuple[bool, str]:
    return base_validator.validate_prompt_against_instructions(
        user_prompt, turn_instructions, language="en"
    )
...
def validate_thinking_against_instructions(thinking: str, turn_instructions: Dict) -> Tuple[bool, str]:
    return base_validator.validate_thinking_against_instructions(
        thinking, turn_instructions, language="en"
    )
